              ...
              <form className="flex flex-col gap-5" onSubmit={(e) => { e.preventDefault(); navigate('/'); }}>
                <label className="flex flex-col gap-1.5">
                  ...
                  <div className="relative group">
                    <div className="absolute left-4 top-1/2 -translate-y-1/2 text-text-muted"><span className="material-symbols-outlined text-[20px]">mail</span></div>
                    <input className="w-full bg-neutral-50 dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-lg h-14 pl-11 pr-4 text-base text-primary dark:text-white focus:ring-2 focus:ring-accent transition-all" placeholder="name@example.com" type="email" required/>
                  </div>
                </label>
                <label className="flex flex-col gap-1.5">
                  <div className="flex justify-between items-center ml-1">
                    <span className="text-primary dark:text-white text-sm font-bold">Password</span>
                    <Link to="/forgot-password" class="text-xs font-semibold text-accent hover:underline">Forgot Password?</Link>
                  </div>
                  <div className="relative group">
                    <div className="absolute left-4 top-1/2 -translate-y-1/2 text-text-muted"><span className="material-symbols-outlined text-[20px]">lock</span></div>
                    <input className="w-full bg-neutral-50 dark:bg-neutral-800 border border-neutral-200 dark:border-neutral-700 rounded-lg h-14 pl-11 pr-11 text-base text-primary dark:text-white focus:ring-2 focus:ring-accent transition-all" placeholder="Enter your password" type="password" required/>
                  </div>
                </label>
                <button className="mt-2 w-full h-14 bg-primary hover:bg-primary-hover text-white font-bold rounded-lg transition-all flex items-center justify-center gap-2 shadow-lg shadow-primary/20" type="submit">
                  Log In <span className="material-symbols-outlined text-[20px]">arrow_forward</span>
                </button>
              </form>
