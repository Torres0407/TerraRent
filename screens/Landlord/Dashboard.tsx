      ...
      <div className="p-8 md:p-12 max-w-7xl mx-auto space-y-12">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
          <div className="space-y-2">
            ...
            <p className="text-text-muted font-medium">Welcome back, Eleanor. Your architectural stays are performing well today.</p>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {[
            { title: "Total Revenue", val: "$12,450", change: "+12%", icon: "payments", color: "green", bg: "#f0fdf4" },
            { title: "Occupancy", val: "92%", change: "+4%", icon: "pie_chart", color: "blue", bg: "#eff6ff" },
            { title: "Active Assets", val: "8", change: "Stable", icon: "house", color: "orange", bg: "#fff7ed", isNeutral: true },
            { title: "New Requests", val: "3", change: "+3 New", icon: "notifications_active", color: "red", bg: "#fef2f2" }
          ].map((stat, i) => (
              ...
              <div className="overflow-x-auto no-scrollbar">
                <table className="w-full text-left">
                  ...
                  <tbody className="text-sm font-medium">
                    {[
                      { prop: "Tuscan Villa", unit: "Main Estate", tenant: "James D.", date: "Oct 24 - 28", status: "Confirmed", color: "green" },
                      { prop: "Aspen Loft", unit: "Upper Wing", tenant: "Michael C.", date: "Nov 01 - 05", status: "Pending", color: "amber" }
                    ].map((row, i) => (
                      <tr key={i} className="hover:bg-sand-light/10 group transition-colors">
                        <td className="py-6 border-b border-primary/5">
                          <span className="block font-black text-primary dark:text-white">{row.prop}</span>
                          <span className="text-xs text-text-muted">{row.unit}</span>
                        </td>
                        <td className="py-6 border-b border-primary/5 text-primary/70">{row.tenant}</td>
                        <td className="py-6 border-b border-primary/5 text-primary/70">{row.date}</td>
                        <td className="py-6 border-b border-primary/5 text-right">
                          <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest ${row.color === 'green' ? 'bg-green-50 text-green-700' : 'bg-amber-50 text-amber-700'}`}>
                            {row.status}
                          </span>
                        </td>
                      </tr>
          ...
          <div className="space-y-8">
            <div className="bg-primary text-white p-10 rounded-[3rem] shadow-2xl shadow-primary/30 relative overflow-hidden">
               ...
               <div className="space-y-6">
                  <div className="flex gap-4 items-start p-6 bg-white/10 rounded-[2rem] border border-white/10">
                     <span className="material-symbols-outlined text-accent">plumbing</span>
                     <div>
                        <p className="font-black text-sm">Mainteance Alert</p>
                        <p className="text-xs text-white/60 mt-1">Water leak detected at Tuscan Villa</p>
                     </div>
                  </div>
                  <div className="flex gap-4 items-start p-6 bg-white/10 rounded-[2rem] border border-white/10">
                     <span className="material-symbols-outlined text-accent">description</span>
                     <div>
                        <p className="font-black text-sm">Lease Renewal</p>
                        <p className="text-xs text-white/60 mt-1">Aspen Loft contract expires in 5 days</p>
                     </div>
                  </div>
               </div>
            </div>
